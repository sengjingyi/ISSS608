...
### Load packages inside, import Exam_data.csv
...
### Visualising Distribution with Histogram

-   Inherent problem with histogram, need to bin data and calculate frequency into the plot.

-   Use of [rgb color](https://www.rapidtables.com/web/color/RGB_Color.html) picker to identify the color code.  `"#xxxxx"`

-   Adjust: Used to set the distribution of the plot

-   Alpha: Used to set the transparency of the color.

```{r}
ggplot(exam_df, aes (x = ENGLISH)) + geom_density(
  color = "#1696d2", 
  adjust = 0.65, 
  alpha = 0.6
)
...
### Using [probability density plot](https://ggplot2.tidyverse.org/reference/geom_density.html) (KDE - Kernel Density Estimation)

Giving shape of distribution (Using a continuous range) to overcome the issue of forced bins for histogram. –\> Consider the underlying dataset nature whether it is continuous or discrete

The alternative design - Overlaying another distribution

```{r}
# Including reference line (mean, median, standard deviation)
median_eng <- median(exam_df$ENGLISH)
mean_eng <- mean(exam_df$ENGLISH)
std_eng <- sd(exam_df$ENGLISH)

# Plotting density plot based on existing data set
# Including a theoretical model (norm distribution, with preset mean and sd)
ggplot(exam_df, aes(x= ENGLISH)) + 
  ...
  linetype = "dashed") + 
  annotate(geom = "text", 
           x = mean_eng - 8, #Include -8 to shift the word beside
           y = 0.04, 
  ...
  linetype = "dashed") + 
  annotate(geom = "text", 
           x = median_eng + 8, #Include -8 to shift the word beside
           y = 0.04, 
