...
4.  Comparing associated vessels with cargo delivered to identify any drastic changes.
...
(i) same vessel delivering multiple cargo of same fish species on the same day to split total volume of fish caught to prevent being flagged for over-fishing,

(ii) same vessel delivering multiple cargo of different fish species on same day, where each cargo is labelled by distinct fish species.
...
datatable(route_vessel_summary, options = list(pageLength = 3))
...
1.  Plot common vessel movement for South Seafood Express Vessels, namely "snappersnatcher7be" and "roachrobberdb6".
...
### 4.2.3 How cargo count have changed
...
tx_count_bef_aft <- ggplot(tx_qty_bef_aft, 
                           aes(x= dest, y = diff_count_bef_aft)) + 
  geom_bar(stat = "identity") + 
  coord_flip()
