...
route_summary <- route %>%
  group_by(vessel_id) %>%
  mutate(route_list = ifelse(report == "hbrpt", list(area), NA),
         route_start = ifelse(report == "hbrpt", start_date, NA),
         route_end = ifelse(report == "hbrpt", lead(start_date), NA)) %>%
  fill(route_list, route_start, route_end, .direction = "down") %>%
  group_by(vessel_id, route_start) %>%
  mutate(route_list = ifelse(is.na(report) | report != "hbrpt", NA, route_list)) %>%
  group_by(vessel_id, route_start) %>%
  mutate(route_list = list(unique(na.omit(route_list[which(start_date >= route_start & start_date <= route_end)])))) %>%
  group_by(vessel_id, route_start) %>%
  summarize(route_list = route_list, 
...
```{r}
# appending other relevant details 
route_list <- route_summary$route_list

# Convert the list to a string of text delimited with commas
route_list_text <- sapply(route_list, function(x) paste(x, collapse = ","))

# Create a new dataframe with the text version of route_list
route_summary_text <- route_summary %>%
  mutate(route_list_text = route_list_text) %>%
  select(-route_list) 

route_summary_text <- route_summary_text %>%
  mutate(route_list_text = gsub("c[(]", "", route_list_text)) %>% 
  mutate(route_list_text = gsub("\"", "", route_list_text)) %>%
  mutate(route_list_text = gsub("[)]", "", route_list_text)) 

#write_csv(route_summary_text, "data/route_summary_text.csv")

# merging with vessel details and potential fish species at the area

# find_possible_species <- function(area, route_summary_text) {
#   possible_species <- c()
#   for (i in 1:nrow(route_summary_text)) {
#     if (area %in% unlist(strsplit(route_summary_text[i, "route_list_text"], ","))) {
#       possible_species <- c(possible_species, route_summary_text[i, "fish_species"])
#     }
#   }
#   return(paste(unique(possible_species), collapse = ", "))
# }
# 
# # Apply the function to each row of dataset 1
# location_legend$fish_species_present <- sapply(location_legend$area, find_possible_species, route_summary_text)
...
-   6 May 2023 for roachrobberdb6 for City of Himark, Cod Table, Wrasse Bed
